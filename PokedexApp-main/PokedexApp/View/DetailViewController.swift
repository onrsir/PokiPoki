...
class DetailViewController: UIViewController {
    ...
    //MARK: - Presentation
        ...
        self.idLabel.text                    = "#\(self.viewModel.convertStringFromOptInt(value: self.viewModel.chosenPokemon?.id))"
            self.weightLabel?.text               = "\(viewModel.formatHeightWeight(value: self.viewModel.chosenPokemon?.weight ?? 0)) KG"
            self.heightLabel?.text               = "\(viewModel.formatHeightWeight(value: self.viewModel.chosenPokemon?.height ?? 0)) M"
